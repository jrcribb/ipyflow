...
class NotebookFlow(singletons.NotebookFlow):
    ...
    def get_position(self, frame: FrameType) -> Tuple[int, int]:
        try:
            ...
            # On Python 3.14+, frame.f_lineno can be None when the frame is
            # positioned at compiler-generated bytecode with no source line
            # (e.g. the implicit cleanup that runs when a @contextmanager
            # generator is resumed past its yield). Such instructions have no
            # corresponding notebook statement, so map them to the lineno == 0
            # sentinel, which downstream handlers already treat as "skip".
            return cell_num, frame.f_lineno or 0
