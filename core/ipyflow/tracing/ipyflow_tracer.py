...
class DataflowTracer(StackFrameManager):
    ...
    def _get_stmt_node_for_sys_event(
        ...
        if lineno == 0:
            # frame.f_lineno was None (Python 3.14+): the frame is at
            # compiler-generated bytecode with no source line, so there is no
            # notebook statement to resolve here
            return None
        try:
    ...
    def handle_other_sys_events(
        ...
        **__,
        ...
        if event != pyc.after_stmt:
            stmt_node = self._get_stmt_node_for_sys_event(event, cell_num, lineno)
            if stmt_node is None and lineno == 0:
                # no notebook statement maps to this synthetic line (e.g. the
                # compiler-generated cleanup that runs when a @contextmanager
                # generator is resumed past its yield); nothing to trace
                return None
        assert stmt_node is not None
